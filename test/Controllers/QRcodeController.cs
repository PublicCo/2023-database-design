...
namespace test.WebAPI.Controllers
{
    ...
    public class QRcodeController : ControllerBase
    {
        ...
        public string AppointmentQRcode()
        {
            ...
            appointment_Infos = DbContext.db.Queryable<appointment_info>().Where(it => it.user_id == id).OrderByDescending(a => a.appoint_time).Take(1).ToList();
            //取最新记录
            ///筛选最新的记录
            appointment_info appointment_Info = appointment_Infos[0];
            data = JsonConvert.SerializeObject(new { code = 200, message = "success", user_id=id, type= appointment_Info.appoint_type });
            return data;
