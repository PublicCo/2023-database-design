...
namespace test.WebAPI.Controllers
{
    ...
    public class ReservevationController : ControllerBase
    {
        ...
        public string GetUserReservation(string hos_id,int type) {
            string data;
            List<appointment_result_dto> Appointment_info_dto = _userService.GetUserReservation(hos_id,type);
        ...
        [HttpPost]
        [Authorize]
        public string ModifyReservation(string hos_id,string time,string user_id,int type,int state)
        {
            string data;
            int result = _userService.ModifyReservation(hos_id, Convert.ToDateTime(time), user_id, type, state);
            if (result > 0)
            {
                data = JsonConvert.SerializeObject(new { code = 200, message = "success" });
                return data;
        ...
        [HttpDelete]
        [Authorize]
        public string CancelReservevation(int type)
        {
            string token;
            string data;
            /*            try
                        ...
                        string id = _jwtService.DecodeToken(token);*/
            string id = _jwtService.GetUserID(HttpContext);
            int result = _userService.CancelReservevation(id, type);
            if (result > 0)
            {
                data = JsonConvert.SerializeObject(new { code = 200, message = "success" });
                return data;
            ...
            {
                data = JsonConvert.SerializeObject(new { code = 400, message = "该条预约已被处理，无法取消" });
                return data;
            }
        }
    }
